...
func @conv_NCHW(%arg0: tensor<5x69x31x95xf32>, %arg1: tensor<64x69x1x1xf32>, %arg2: tensor<64xf32>) -> tensor<5x64x31x95xf32> {
...
func @conv_backward_data(%arg0: tensor<32x64x56x56xf16>, %arg1: tensor<64x64x3x3xf16>) -> tensor<32x64x56x56xf16> {
  %0 = "mhlo.fusion"(%arg0, %arg1) ({
    ...
    %3 = mhlo.convolution(%arg0, %2) dim_numbers = [b, f, 0, 1]x[0, 1, o, i]->[b, f, 0, 1], window = {stride = [1, 1], pad = [[1, 1], [1, 1]], lhs_dilate = [1, 1], rhs_dilate = [1, 1]} {batch_group_count = 1 : i64, feature_group_count = 1 : i64, precision_config = ["DEFAULT", "DEFAULT"]} : (tensor<32x64x56x56xf16>, tensor<3x3x64x64xf16>) -> tensor<32x64x56x56xf16>
...
func @pool_grad_NCHW(%arg83: tensor<1x64x112x112xf16>, %76: tensor<1x64x56x56xf16>) -> tensor<1x64x112x112xf16> {
...
// CHECK-LABEL: func @pool_grad_NCHW
// CHECK-NEXT:  mhlo.constant
// CHECK-NEXT:  mhlo.transpose{{.*}}permutation = dense<[0, 2, 3, 1]>
// CHECK-NEXT:  mhlo.transpose{{.*}}permutation = dense<[0, 2, 3, 1]>
...
func @conv3d_NCDHW(%140: tensor<1x3x100x27x48xf32>, %16: tensor<32x3x1x3x3xf32>) -> tensor<1x32x100x27x48xf32> {
...
func @avg_pool3d_NDHWC(%187: tensor<1x100x27x48x64xf32>) -> tensor<1x100x13x24x64xf32> {
...
// NDHWC-LABEL: func @avg_pool3d_NDHWC
// NDHWC-NOT: mhlo.transpose

func @avg_pool3d_NCDHW(%187: tensor<1x64x100x27x48xf32>) -> tensor<1x64x100x13x24xf32> {
...
// NDHWC-LABEL: func @avg_pool3d_NCDHW
