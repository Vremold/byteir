...
// CHECK: 	0 : index
// CHECK: for operation : %0 = tensor.from_elements %c1_i32, %c2_i32, %c0_i32 : tensor<3xi32>, inferred values are:
// CHECK: 	dense<[1, 2, 0]> : tensor<3xi32>
// CHECK: for operation : %1 = "tosa.transpose"(%arg0, %0) : (tensor<3x4x5xi32>, tensor<3xi32>) -> tensor<?x?x?xi32>, inferred shapes are:
// CHECK: 	tensor<4x5x3xi32>
// CHECK: for operation : %1 = "tosa.transpose"(%arg0, %0) : (tensor<3x4x5xi32>, tensor<3xi32>) -> tensor<?x?x?xi32>, inferred values are:
// CHECK: 	<NO VALUE>
// CHECK: for operation : %2 = "tosa.add"(%1, %1) : (tensor<?x?x?xi32>, tensor<?x?x?xi32>) -> tensor<?x?x?xi32>, inferred shapes are:
// CHECK: 	tensor<4x5x3xi32>
// CHECK: for operation : %2 = "tosa.add"(%1, %1) : (tensor<?x?x?xi32>, tensor<?x?x?xi32>) -> tensor<?x?x?xi32>, inferred values are:
// CHECK: 	<NO VALUE>
// CHECK: for operation : %3 = shape.shape_of %arg0 : tensor<3x4x5xi32> -> tensor<3xindex>, inferred values are:
// CHECK: 	dense<[3, 4, 5]> : tensor<3xindex>
// CHECK: for operation : %4 = shape.shape_of %2 : tensor<?x?x?xi32> -> tensor<3xindex>, inferred values are:
// CHECK: 	dense<[4, 5, 3]> : tensor<3xindex>
// CHECK: for operation : %5 = tensor.extract %4[%c0] : tensor<3xindex>, inferred values are:
// CHECK: 	4 : index
// CHECK: for operation : %6 = tensor.dim %2, %c0 : tensor<?x?x?xi32>, inferred values are:
// CHECK: 	4 : index
// CHECK: for operation : %7 = arith.addi %5, %6 : index, inferred values are:
// CHECK: 	8 : index
...
func.func @test_dynamic_callgraph(%arg : tensor<3x4x5xi32>) -> () {
  ...
  %new_shape = tensor.from_elements %c1, %c2, %c0 : tensor<3xi32>
  %1 = "tosa.transpose"(%arg, %new_shape) : (tensor<3x4x5xi32>, tensor<3xi32>)  -> (tensor<?x?x?xi32>)
  %2 = call @test_dynamic_callgraph_inner(%1) : (tensor<?x?x?xi32>) -> tensor<?x?x?xi32>
  %3 = shape.shape_of %2 : tensor<?x?x?xi32> -> tensor<3xindex>
// CHECK: for operation : %3 = shape.shape_of %2 : tensor<?x?x?xi32> -> tensor<3xindex>, inferred values are:
// CHECK: 	dense<[4, 5, 3]> : tensor<3xindex>
  return
}
...
func.func @test_dynamic_with_cf(%arg : tensor<3x4x5xi32>, %cond : i1) -> () {
  %c0 = arith.constant 0 : i32
  %c1 = arith.constant 1 : i32
  %c2 = arith.constant 2 : i32
  %new_shape = tensor.from_elements %c1, %c2, %c0 : tensor<3xi32>
  %1 = "tosa.transpose"(%arg, %new_shape) : (tensor<3x4x5xi32>, tensor<3xi32>)  -> (tensor<?x?x?xi32>)
  cf.cond_br %cond, ^bb1(%1: tensor<?x?x?xi32>), ^bb2(%1 : tensor<?x?x?xi32>)

^bb1(%arg1: tensor<?x?x?xi32>):
  %2 = "tosa.add"(%arg1, %arg1) : (tensor<?x?x?xi32>, tensor<?x?x?xi32>) -> tensor<?x?x?xi32>
...
^bb2(%arg2 : tensor<?x?x?xi32>):
  %3 = shape.shape_of %arg2 : tensor<?x?x?xi32> -> tensor<3xindex>
// CHECK: for operation : %5 = shape.shape_of %4 : tensor<?x?x?xi32> -> tensor<3xindex>, inferred values are:
// CHECK: 	dense<[4, 5, 3]> : tensor<3xindex>
  return
}
...
func.func @test_cf_join_same(%arg : tensor<3x4x5xi32>, %cond : i1) -> () {
  %c0 = arith.constant 0 : i32
  %c1 = arith.constant 1 : i32
  %c2 = arith.constant 2 : i32
  %new_shape = tensor.from_elements %c1, %c2, %c0 : tensor<3xi32>
...
^bb2(%arg2 : tensor<?x?x?xi32>):
  %3 = "tosa.sub"(%arg2, %arg2) : (tensor<?x?x?xi32>, tensor<?x?x?xi32>) -> tensor<?x?x?xi32>
// CHECK: for operation : %5 = "tosa.sub"(%4, %4) : (tensor<?x?x?xi32>, tensor<?x?x?xi32>) -> tensor<?x?x?xi32>, inferred shapes are:
// CHECK: 	tensor<4x5x3xi32>
  cf.br ^bb3(%3: tensor<?x?x?xi32>)

^bb3(%arg3 : tensor<?x?x?xi32>):
  %4 = shape.shape_of %arg3 : tensor<?x?x?xi32> -> tensor<3xindex>
// CHECK: for operation : %7 = shape.shape_of %6 : tensor<?x?x?xi32> -> tensor<3xindex>, inferred values are:
// CHECK: 	dense<[4, 5, 3]> : tensor<3xindex>
  return
}
...
func.func @test_cf_join_partial_shape(%arg0 : tensor<3x4x5xi32>, %arg1 : tensor<3x4x6xi32>, %cond : i1) -> () {
...
// CHECK:   tensor<3x4x6xi32>
  cf.cond_br %cond, ^bb(%0: tensor<?x?x?xi32>), ^bb(%1 : tensor<?x?x?xi32>)

^bb(%arg2 : tensor<?x?x?xi32>):
  %2 = "tosa.add"(%arg2, %arg2) : (tensor<?x?x?xi32>, tensor<?x?x?xi32>) -> tensor<?x?x?xi32>
// CHECK: for operation : %3 = "tosa.add"(%2, %2) : (tensor<?x?x?xi32>, tensor<?x?x?xi32>) -> tensor<?x?x?xi32>, inferred shapes are:
// CHECK: 	tensor<3x4x?xi32>
  %c0 = arith.constant 0 : index
  %3 = tensor.dim %2, %c0 : tensor<?x?x?xi32>
// CHECK: for operation : %4 = tensor.dim %3, %c0 : tensor<?x?x?xi32>, inferred values are:
// CHECK: 	3 : index
  return
}
...
func.func private @test_callgraph_join_partial_shape_inner(%arg: tensor<?x?x?xi32>) -> tensor<?x?x?xi32> {
  %0 = "tosa.add"(%arg, %arg) : (tensor<?x?x?xi32>, tensor<?x?x?xi32>) -> tensor<?x?x?xi32>
// CHECK: for operation : %0 = "tosa.add"(%arg0, %arg0) : (tensor<?x?x?xi32>, tensor<?x?x?xi32>) -> tensor<?x?x?xi32>, inferred shapes are:
// CHECK:   tensor<3x4x?xi32>
  %c0 = arith.constant 0 : index
  %1 = tensor.dim %0, %c0 : tensor<?x?x?xi32>
// CHECK: for operation : %1 = tensor.dim %0, %c0 : tensor<?x?x?xi32>, inferred values are:
// CHECK: 	3 : index
  return %0: tensor<?x?x?xi32>
}
