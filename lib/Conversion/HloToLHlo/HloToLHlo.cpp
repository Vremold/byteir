...
/// InsertAlloc(Location loc, OpResult result, ConversionPatternRewriter* rewriter)
/// Simply remap result_type == result.getType(), def_op == result.getDefiningOp()
...
/// LWC: this one modified the upstream one.
/// InsertDynamicAlloc(Location loc, Value result, Value shape_operand, ConversionPatternRewriter* rewriter)
/// Simply remap result_type == result.getType(), def_op == result.getDefiningOp()
...
struct HloToLhloReduceWindowOpConverter : public BaseOpConversion<mhlo::ReduceWindowOp> {
public:
  ...
  matchAndRewrite(mhlo::ReduceWindowOp op, mhlo::ReduceWindowOpAdaptor adaptor,
    ...
    SmallVector<Value, 4> buffer_args(operands.begin(), operands.end());
    if (failed(ConvertResults(op, buffer_args, rewriter))) return failure();
    auto new_op = rewriter.create<lmhlo::ReduceWindowOp>(loc, llvm::None, buffer_args,
      op->getAttrs());
