...
static std::string GetOutlineFuncitonName(mhlo::FusionOp fusionOp, unsigned& cnt) {
  StringAttr nameAttr = fusionOp->getAttrOfType<StringAttr>(byre::getByreComputeName());
  std::string funcName;
...
static FuncOp CreateOutlinedFuncOp(mhlo::FusionOp fusionOp, StringRef funcName) {
  ...
  opBuilder.setInsertionPoint(block, block->end()); // the point set at the end of block
  opBuilder.create<mlir::ReturnOp>(terminator->getLoc(), terminator->getOperands());
...
static void RewriteFusionOpToCall(mhlo::FusionOp fusionOp, FuncOp funcOp) {
  ...
  auto callOp = opBuilder.create<mlir::CallOp>(fusionOp.getLoc(), funcOp, fusionOp.getOperands());
...
struct FusionOutliningPass
  : public FusionOutliningBase<FusionOutliningPass> {
...
mlir::createFusionOutliningPass() {
