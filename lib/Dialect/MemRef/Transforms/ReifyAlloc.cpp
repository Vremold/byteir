...
struct ReifyAllocPattern : public OpRewritePattern<OpTy> {
  ...
  LogicalResult matchAndRewrite(OpTy opTy,
    ...
    auto oldMemRef = opTy->getResult(0).getType().template dyn_cast<MemRefType>();
    reifyAllocLikeShapeAndOperands(oldMemRef.getShape(), opTy->getOperands(), newShape, newOperands);

    MemRefType newMmRefType =
      MemRefType::get(newShape, oldMemRef.getElementType(), oldMemRef.getLayout(), oldMemRef.getMemorySpace());

    auto newOp = rewriter.replaceOpWithNewOp<OpTy>(opTy, newMmRefType, newOperands);
...
void mlir::reifyAllocLikeShapeAndOperands(
  ...
  for (size_t i = 0; i < oldShape.size(); ++i) {
    ...
    if (dim >= 0) {
      ...
      if (auto cOp = dyn_cast_or_null<arith::ConstantIndexOp>(val.getDefiningOp())) {
...
void mlir::populateReifyAllocLikePatterns(RewritePatternSet& patterns) {
  patterns.add<
    ReifyAllocPattern<memref::AllocOp>,
    ReifyAllocPattern<memref::AllocaOp>
  >(patterns.getContext());
}
