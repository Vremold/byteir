...
namespace {
  llvm::Optional<mlir::AffineMap> createTestAffineMap(MLIRContext* ctx, mlir::MemRefType memref) {
    if (memref.getRank() == 2) {
...
llvm::Optional<mlir::AffineMap> mlir::createDefaultAffineMap(MLIRContext* ctx, mlir::MemRefType memref) {
  return AffineMap::get(memref.getRank(),
      memref.getNumDynamicDims(), ctx);
}
...
llvm::Optional<llvm::StringRef> mlir::getLayoutName(mlir::Value val) {
  ...
  if (auto defOp = val.getDefiningOp()) {
    if (defOp->hasAttrOfType<StringAttr>(getLayoutAttributeName())) {
      return defOp->getAttrOfType<StringAttr>(getLayoutAttributeName()).getValue();
    }
  } else if (auto arg = val.dyn_cast<BlockArgument>()){
    Region* region = arg.getParentRegion();
    if (region == nullptr) return llvm::None;
