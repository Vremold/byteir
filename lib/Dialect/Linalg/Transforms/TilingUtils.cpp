...
LogicalResult mlir::buildSCFLoop(OpBuilder& b, Location loc,
  bool isParallel,
  ValueRange lbs,
  ValueRange ubs, 
  ValueRange steps,
...
LogicalResult mlir::buildAffineLoop(OpBuilder& b, Location loc,
  ValueRange lbs,
  ValueRange ubs, 
  ValueRange steps,
...
Optional<linalg::LinalgOp> mlir::createAtomicLinalgGeneric(
  ...
  linalg::LinalgOp linalgOp = b.create<linalg::GenericOp>(
    loc, inputRange, outputRange, indexingMaps,
    parallelLoopAttrs,
  ...
  linalgOp->setAttr(getAtomicKindAttrName(), 
