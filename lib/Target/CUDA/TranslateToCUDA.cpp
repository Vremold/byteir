//===- TranslateToCUDA.cpp - Translating to CUDA calls --------------------===//
//
...
namespace {
  static LogicalResult printOperation(CUDAEmitter& emitter, GPUFuncOp functionOp) {
    ...
    if (!functionOp.empty()) os << "__global__ ";
    ...
    if (functionOp.empty()) {
      ...
      if (failed(interleaveCommaWithError(
        functionOp.getType().getInputs(), os,
  ...
  static LogicalResult printOperation(CUDAEmitter& emitter, ModuleOp moduleOp) {
    ...
    if (emitter.shouldEmitKernelOnly()) {
      for (Operation& op : moduleOp) {
        if (!isa<GPUModuleOp>(op)) continue;
  ...
  static LogicalResult printOperation(CUDAEmitter& emitter, GPUModuleOp moduleOp) {
    CppEmitter::Scope scope(emitter);

    if (emitter.shouldEmitKernelOnly()) {
      for (Operation& op : moduleOp) {
        if (!isa<GPUFuncOp>(op)) continue;
        if (failed(emitter.emitOperation(op, /*trailingSemicolon=*/false))) {
          return failure();
        }
      }
  ...
  static LogicalResult printOperation(CUDAEmitter& emitter, gpu::ReturnOp returnOp) {
    raw_ostream& os = emitter.ostream();
    os << "return";
...
CUDAEmitter::CUDAEmitter(raw_ostream& os, 
  bool declareVariablesAtTop, bool kernelOnly, bool externC)
  : CppEmitter(os, declareVariablesAtTop), 
    kernelOnly(kernelOnly), externC(externC) {}
...
LogicalResult CUDAEmitter::emitOperation(Operation& op, bool trailingSemicolon) {
  ...
  LogicalResult status =
    ...
    // GPU ops.
    .Case<GPUModuleOp, GPUFuncOp, 
          GridDimOp, BlockDimOp, BlockIdOp, ThreadIdOp,
          BarrierOp, gpu::ReturnOp>(
       [&](auto op) { return printOperation(*this, op); })
    // bubble ops
    .Case<gpu::ModuleEndOp>(
       [&](auto) {
         earlyTerminate = true;
         return success(); })
  ...
  if (earlyTerminate) return success();
  if (callEmitter) return CppEmitter::emitOperation(op, trailingSemicolon);
