...
void BertTransformer<OpType_>::bert_infer(BertTransformerInferParam infer_param)
{
    const DataType_* from_tensor = infer_param.input_tensor;    //Todo: remove useless code
    const DataType_* to_tensor   = infer_param.input_tensor;
    ...
    DataType_* attr_matmul_buf_  = inner_buf + 4 * input_tensor_size;
    ...
    if(is_remove_padding_)
    {
        ...
        compressBertInput_kernelLauncher(
                        from_tensor, transformer_out,
                        et_param.batch_idx, et_param.word_idx, valid_word_num,
                        batch_size, hidden_dim, stream);

        from_tensor = transformer_out;      //1. compress from_tensor      -> transformert_out
        to_tensor   = transformer_out;
    ...
    dense_layer_kernel_launcher(
                    from_tensor, param_.attr_kernel_Q, query_buf_,
                    m, k, n, cublas_handle, stream, param_.cublas_Algo[0]);

    dense_layer_kernel_launcher(
                    to_tensor, param_.attr_kernel_K, key_buf_,
                    m, k, n, cublas_handle, stream, param_.cublas_Algo[0]);

    dense_layer_kernel_launcher(
                    to_tensor, param_.attr_kernel_V, value_buf_,
                    m, k, n, cublas_handle, stream, param_.cublas_Algo[0]);
    ...
    add_bias_input_layernorm_kernel_launcher(
                    attr_matmul_buf_, from_tensor, param_.attr_output_bias,
    ...
    cudaMemsetAsync(is_remove_padding_ ? inner_buf : transformer_out,
    ...
    if(is_remove_padding_)
        add_bias_input_layernorm_restore_output_kernel_launcher(
                        transformer_out, attr_matmul_buf_, param_.output_bias,
...
template void BertTransformer<OperationType::FP32>::bert_infer(BertTransformerInferParam infer_param);
template void BertTransformer<OperationType::HALF>::bert_infer(BertTransformerInferParam infer_param);
}
