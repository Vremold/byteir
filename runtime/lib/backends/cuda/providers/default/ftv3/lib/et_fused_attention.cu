...
__global__
    ...
    void
    et_wmma_attention_kernel(const half2 *query, const half2 *query_bias,
                             const half2 *key, const half2 *key_bias,
                             const half2 *value, const half2 *value_bias,
                             const __half *attention_mask,
                             __half *attention_output, const int *batch_idx,
                             const int *word_idx, const int batch_size,
  ...
  const int batch_offset = __ldg(&batch_idx[bid]);
  const int batch_seq_len = __ldg(&batch_idx[bid + 1]) - batch_offset;

  const int from_size = (batch_seq_len + 15) >> 4;
  const int to_size = (batch_seq_len + 15) >> 4;

  const int half_hidden_dim = head_num * (size_per_head / 2);
  const int head_offset = head_id * (size_per_head / 2);
  const int bias_id = head_offset + warp_tid;

  // loading Query & Key
  for (int seq_id = warpId; seq_id < batch_seq_len; seq_id += warpNums) {
    int pos =
        (batch_offset + seq_id) * half_hidden_dim + head_offset + warp_tid;
...
#pragma unroll
    for (int i = 0; i < n; i++) {
      ...
      logits[i] = to_id[i] < batch_seq_len
                      ? (float)(s_logits[from_id][to_id[i]])
                      : -1e20f;
    }
    float max_val = max(logits[0], logits[1]);
...
#pragma unroll
    for (int i = 0; i < n; i++) {
      ...
      sum_val += (to_id[i] < batch_seq_len) ? logits[i] : 0.0f;
    }
...
#pragma unroll
    for (int i = 0; i < n; i++)
      if (to_id[i] < max_seq_len)
        s_logits[from_id][to_id[i]] =
            (__half)(to_id[i] < batch_seq_len ? (logits[i] / (sum_val + 1e-6f))
                                              : 0.0f);
  }
  ...
  // K dim clear 0
  for (int seq_id = batch_seq_len + warpId; seq_id < max_seq_len;
       seq_id += warpNums)
...
#pragma unroll
  ...
  for (int from_id = warpId; from_id < batch_seq_len; from_id += warpNums) {
    int pos =
        (batch_offset + from_id) * half_hidden_dim + head_offset + warp_tid;
...
template <const int max_seq_len, const int size_per_head>
__global__
    // __launch_bounds__(256,8)//THREADS_PER_BLOCK
    void
    et_wmma_attention_kernel_LE32(const half2 *query, const half2 *query_bias,
                                  const half2 *key, const half2 *key_bias,
                                  const half2 *value, const half2 *value_bias,
                                  const __half *attention_mask,
                                  ...
                                  const int seq_len, const half2 scaler) {
  __shared__ __half s_kv[max_seq_len][size_per_head + SKEW_HALF];
  __shared__ __half s_query[max_seq_len][size_per_head + SKEW_HALF];
  __shared__ __half s_logits[max_seq_len][max_seq_len + SKEW_HALF];
...
#pragma unroll
  ...
  // softmax
  for (int from_id = warpId; from_id < batch_seq_len; from_id += warpNums) {
    float logits = warp_tid < batch_seq_len
                       ? (float)(s_logits[from_id][warp_tid])
                       : -1e20f;
    float max_val = warpReduceMax(logits);
    logits = __expf(logits - max_val);
    float sum_val = (warp_tid < batch_seq_len) ? logits : 0.0f;
    sum_val = warpReduceSum(sum_val);
    if (warp_tid < max_seq_len)
      s_logits[from_id][warp_tid] =
          (__half)(warp_tid < batch_seq_len ? logits / (sum_val + 1e-6f)
                                            : 0.0f);
  }
...
void Attention<OpType_>::et_fused_infer(
  ...
  if (OpType_ == OperationType::HALF) {
    ...
    if (seq_len <= 16) {
      block.x = 128;
      et_wmma_attention_kernel_LE32<16, 64><<<grid, block, 0, stream>>>(
          query_ptr, query_bias_ptr, key_ptr, key_bias_ptr, value_ptr,
          value_bias_ptr, (__half *)atten_mask, (__half *)attention_output,
          et_param.batch_idx, et_param.word_idx, batch_size, head_num_, seq_len,
          scaler);
    } else if (seq_len <= 32) {
      block.x = 256;
      et_wmma_attention_kernel_LE32<32, 64><<<grid, block, 0, stream>>>(
          query_ptr, query_bias_ptr, key_ptr, key_bias_ptr, value_ptr,
          value_bias_ptr, (__half *)atten_mask, (__half *)attention_output,
          et_param.batch_idx, et_param.word_idx, batch_size, head_num_, seq_len,
          scaler);
      ...
      block.x = 384;
      et_wmma_attention_kernel<48, 64><<<grid, block, 0, stream>>>(
          query_ptr, query_bias_ptr, key_ptr, key_bias_ptr, value_ptr,
          value_bias_ptr, (__half *)atten_mask, (__half *)attention_output,
          et_param.batch_idx, et_param.word_idx, batch_size, head_num_, seq_len,
          scaler);
      ...
      block.x = 512;
      et_wmma_attention_kernel<64, 64><<<grid, block, 0, stream>>>(
          query_ptr, query_bias_ptr, key_ptr, key_bias_ptr, value_ptr,
          value_bias_ptr, (__half *)atten_mask, (__half *)attention_output,
          et_param.batch_idx, et_param.word_idx, batch_size, head_num_, seq_len,
          scaler);
...
template void Attention<OperationType::FP32>::et_fused_infer(
    const float *query, const float *key, const float *value,
    const float *atten_mask, float *attention_output, void *buf,
    const int batch_size, const int seq_len, cublasHandle_t cublas_handle,
    cudaStream_t stream, ET_Param et_param);

template void Attention<OperationType::HALF>::et_fused_infer(
