...
const void *CreateBertTransformer(brt::ir::ByREBuilder &byre_builder) {
  ...
  auto space_attr = StringAttr::get(ctx, "cuda");
  llvm::SmallVector<int64_t, 4> shape_attr_kernel = {hidden, hidden};
  auto type_arg_attr_kernel =
      MemRefType::get(shape_attr_kernel, op_builder.getF32Type(),
                      MemRefLayoutAttrInterface{}, space_attr);

  llvm::SmallVector<int64_t, 4> shape_attr_bias = {hidden};
  auto type_arg_attr_bias = MemRefType::get(
      hidden, op_builder.getF32Type(), MemRefLayoutAttrInterface{}, space_attr);

  llvm::SmallVector<int64_t, 4> shape_attr_out_kernel = {hidden, hidden};
  auto type_arg_attr_out_kernel =
      MemRefType::get(shape_attr_out_kernel, op_builder.getF32Type(),
                      MemRefLayoutAttrInterface{}, space_attr);

  llvm::SmallVector<int64_t, 4> shape_attr_out_bias = {hidden};
  auto type_arg_attr_out_bias =
      MemRefType::get(shape_attr_out_bias, op_builder.getF32Type(),
                      MemRefLayoutAttrInterface{}, space_attr);

  llvm::SmallVector<int64_t, 4> shape_attr_layernorm = {hidden};
  auto type_attr_layernorm =
      MemRefType::get(shape_attr_layernorm, op_builder.getF32Type(),
                      MemRefLayoutAttrInterface{}, space_attr);

  llvm::SmallVector<int64_t, 4> shape_inter_kernel = {hidden, 4 * hidden};
  auto type_inter_kernel =
      MemRefType::get(shape_inter_kernel, op_builder.getF32Type(),
                      MemRefLayoutAttrInterface{}, space_attr);

  llvm::SmallVector<int64_t, 4> shape_inter_bias = {4 * hidden};
  auto type_inter_bias =
      MemRefType::get(shape_inter_bias, op_builder.getF32Type(),
                      MemRefLayoutAttrInterface{}, space_attr);

  llvm::SmallVector<int64_t, 4> shape_out_kernel = {4 * hidden, hidden};
  auto type_out_kernel =
      MemRefType::get(shape_out_kernel, op_builder.getF32Type(),
                      MemRefLayoutAttrInterface{}, space_attr);

  llvm::SmallVector<int64_t, 4> shape_out_bias = {hidden};
  auto type_out_bias = MemRefType::get(shape_out_bias, op_builder.getF32Type(),
                                       MemRefLayoutAttrInterface{}, space_attr);

  llvm::SmallVector<int64_t, 4> shape_out_layernorm = {hidden};
  auto type_out_layernorm =
      MemRefType::get(shape_out_layernorm, op_builder.getF32Type(),
                      MemRefLayoutAttrInterface{}, space_attr);

  llvm::SmallVector<int64_t, 4> shape_input = {batch, seq, hidden};
  auto type_input = MemRefType::get(shape_input, op_builder.getF32Type(),
                                    MemRefLayoutAttrInterface{}, space_attr);

  llvm::SmallVector<int64_t, 4> shape_att_mask = {batch, seq, seq};
  auto type_att_mask = MemRefType::get(shape_att_mask, op_builder.getF32Type(),
                                       MemRefLayoutAttrInterface{}, space_attr);

  llvm::SmallVector<int64_t, 4> shape_output = {batch, seq, hidden};
  auto type_output = MemRefType::get(shape_output, op_builder.getF32Type(),
                                     MemRefLayoutAttrInterface{}, space_attr);

  // create an entry func
  func::FuncOp func_op = byre_builder.CreateEntryPointFuncSignature(
      "test", {{type_arg_attr_kernel, AT::Weight, "attr_kernel_q"},
               {type_arg_attr_kernel, AT::Weight, "attr_kernel_k"},
               {type_arg_attr_kernel, AT::Weight, "attr_kernel_v"},
               {type_arg_attr_bias, AT::Weight, "attr_bias_q"},
               {type_arg_attr_bias, AT::Weight, "attr_bias_k"},
               {type_arg_attr_bias, AT::Weight, "attr_bias_v"},
               {type_arg_attr_out_kernel, AT::Weight, "attr_output_kernel"},
               {type_arg_attr_out_bias, AT::Weight, "attr_output_bias"},
               {type_attr_layernorm, AT::Weight, "attr_output_layernorm_beta"},
               {type_attr_layernorm, AT::Weight, "attr_output_layernorm_gamma"},
               {type_inter_kernel, AT::Weight, "inter_kernel"},
               {type_inter_bias, AT::Weight, "inter_bias"},
               {type_out_kernel, AT::Weight, "output_kernel"},
               {type_out_bias, AT::Weight, "output_bias"},
               {type_out_layernorm, AT::Weight, "output_layernorm_beta"},
               {type_out_layernorm, AT::Weight, "output_layernorm_gamma"},
               {type_input, AT::Input, "input_tensor"},
               {type_att_mask, AT::Input, "atten_mask"},
               {type_output, AT::Output, "output"}});
  ...
  SmallVector<Value> inputs{
      entry_block->getArgument(16), entry_block->getArgument(17),
      entry_block->getArgument(0),  entry_block->getArgument(1),
      entry_block->getArgument(2),  entry_block->getArgument(3),
      entry_block->getArgument(4),  entry_block->getArgument(5),
      entry_block->getArgument(6),  entry_block->getArgument(7),
      entry_block->getArgument(8),  entry_block->getArgument(9),
      entry_block->getArgument(10), entry_block->getArgument(11),
      entry_block->getArgument(12), entry_block->getArgument(13),
      entry_block->getArgument(14), entry_block->getArgument(15),
      entry_block->getArgument(16), entry_block->getArgument(17)};
  SmallVector<Value> outputs{entry_block->getArgument(18)};
  auto op = op_builder.create<byre::ComputeOp>(
      UnknownLoc::get(ctx), "BertTransformerOp", inputs, outputs);

  op->setAttr("head_num", op_builder.getI64IntegerAttr(32));
  op->setAttr("size_per_head", op_builder.getI64IntegerAttr(16));
